...
public class TopPlayersFragment extends Fragment {
    ...
    public void loadTopPlayers(final View v){
        ...
        call = conexion.getTopPlayersRequest("Bearer "+globalToken);
            ...
            public void onResponse(Call<List<TopPlayers>> call, Response<List<TopPlayers>> response) {
                ...
                listOfTopPlayersToSend = new ArrayList<>();
                    ...
                    for (int i = 0; i < auxiliarList.size(); i++) {
                        ...
                        if (auxiliarList.get(i).getGame().equals(actualGame)) {

                            if (auxiliarList.get(i).getAvatar() == null || auxiliarList.get(i).getAvatar() == "" || auxiliarList.get(i).getAvatar() == " ") {
                                avatar = null;
                            } else {
                                avatar = auxiliarList.get(i).getAvatar();
                            }

                            if (auxiliarList.get(i).getName() == null) {
                                name = "Sin nombre";
                            } else {
                                name = auxiliarList.get(i).getName();
                            }

                            if (auxiliarList.get(i).getBiografia() == null) {
                                biografia = "Sin biografia";
                            } else {
                                biografia = auxiliarList.get(i).getBiografia();
                            }

                            if (auxiliarList.get(i).getGame() == null) {
                                game = "Sin juego";
                            } else {
                                game = auxiliarList.get(i).getGame();
                            }
